...
public class StreamApi {
    public static void main(String[] args) {
        ...
        stringStream.filter(x -> x.equals("one")).forEach(System.out::println);//only once
        ...
        Stream.of("one", "two").anyMatch(x -> x.equals("one"));
        Stream.of(1, 2).mapToInt(x -> x).sum();
        Stream.of(1, 2).count();
        Stream.of("one", "two").peek(String::toUpperCase).findFirst().get();
